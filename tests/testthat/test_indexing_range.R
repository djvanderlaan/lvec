...
test_that("numeric vectors are correctly indexed using lget", {
  ...
  # regular index
  expect_that(as.rvec(lget(a, range=c(1,4))), equals(a_r[1:4]))
  expect_that(as.rvec(lget(a, range=c(3,4))), equals(a_r[3:4]))
  expect_that(as.rvec(lget(a, range=c(3,3))), equals(a_r[3:3]))
  # invalid ranges
