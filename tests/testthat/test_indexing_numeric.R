...
test_that("numeric vectors are correctly indexed using lget", {
  ...
  # regular index
  expect_that(as.rvec(lget(a, 1:4)), equals(a_r[1:4]))
  expect_that(as.rvec(lget(a, 3:4+0.1)), equals(a_r[3:4+0.1]))
  expect_that(as.rvec(lget(a, 3:2)), equals(a_r[3:2]))
  # index with missing values
  expect_that(as.rvec(lget(a, c(1,NA))), equals(a_r[c(1,NA)]))
  # empty index
  expect_that(as.rvec(lget(a, numeric(0))), equals(a_r[numeric(0)]))
  # out of range
