...
test_that("numeric vectors are correctly indexed using lget", {
  ...
  # regular index
  expect_that(as.rvec(lget(a, c(TRUE, FALSE, FALSE, TRUE))), equals(a_r[c(TRUE, FALSE, FALSE, TRUE)]))
  expect_that(as.rvec(lget(a, c(FALSE, TRUE))), equals(a_r[c(FALSE, TRUE)]))
  expect_that(as.rvec(lget(a, c(TRUE))), equals(a_r[c(TRUE)]))
  expect_that(as.rvec(lget(a, c(FALSE))), equals(a_r[c(FALSE)]))
  # index with missing values
  expect_that(as.rvec(lget(a, c(TRUE, NA, FALSE, TRUE))), equals(a_r[c(TRUE, NA, FALSE, TRUE)]))
  # empty index
  expect_that(as.rvec(lget(a, logical(0))), equals(a_r[logical(0)]))
})
